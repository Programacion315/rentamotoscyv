...
export function WhatsAppFab({ href }: { href: string }) {
  return (
    <a
      ...
      aria-label="Reserva con nosotros en WhatsApp"
      className={cn(
        "group/wa fixed z-[60] flex items-center justify-center rounded-full",
        "bg-[#25D366] text-white",
        "right-5 bottom-5 h-14 w-14",
        "shadow-[0_4px_24px_rgba(37,211,102,0.5)]",
        "transition-[transform,box-shadow] duration-300 ease-out",
        "hover:scale-105 hover:shadow-[0_8px_32px_rgba(37,211,102,0.55)]",
        "md:right-8 md:bottom-8 md:h-16 md:w-16"
      )}
    >
      <span
        aria-hidden
      ...
      <span
        aria-hidden
        className="wa-pulse-delayed pointer-events-none absolute inset-0 rounded-full bg-[#25D366]"
      />

      <svg
        className="relative z-[1] h-7 w-7 fill-current md:h-8 md:w-8"
        viewBox="0 0 24 24"
        aria-hidden
      >
      ...
      >
        Reserva con nosotros en WhatsApp
        <span
          aria-hidden
          className="absolute top-1/2 left-full -translate-y-1/2 border-[6px] border-transparent border-l-black"
        />
      </span>
    </a>
  )
